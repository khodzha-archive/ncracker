 ...
 * к полям и методам объектов с помощью Reflection API.
 * Дополнительно: научиться упаковывать данные в коллекции (с Generics). 
 * 
 * ЗАДАНИЕ
 * Реализовать методы данного интерфейса путевы вызова Reflection API
...
public interface Reflections {
	...
	/**
	 * Метод возвращает набор всех методов, в т.ч. методов суперклассов 
	 * @param clazz - класс
	 * @throws NullPointerException - если claszz является null-ом
	 * @return Набор методов для всей иерархии. 
	 * */	
	...
	/**
	 * Мотод создает экземпляр класса SecretClass с помощью конструктора по умолчанию, 
	...
	/**
	 * Мотод создает экземпляр класса SecretClass с помощью конструктора с параметром constructorParameter 
